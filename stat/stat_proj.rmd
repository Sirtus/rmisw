...
test <- !train
prices_vs_v <- regsubsets(SalePrice ~ ., data = data_clean[train,], nvmax = 12, method="forward")
...
prediction_error <- function(i, model, subset) {
  pred <- predict(model, data_clean[subset,], id = i)
  mean((data_clean$SalePrice[subset] - pred)^2)
}
val_errors <- sapply(1:10, prediction_error, model = prices_vs_v, subset =test)
val_errors
```
